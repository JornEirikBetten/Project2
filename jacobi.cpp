...
void init(int n, double h, mat& a, mat &v) {
	for (int i=0;i<n;i++) {
		for (int j=0;j<n;j++) {
			if(i==j) {
				a(i,j)=2/(h*h); 
...
void find_lowest_eigenvalue (mat a, int n, int &lowest) {
...
void write_eig_to_file(mat v, int n, int lowest, string filename){
	ofstream ofile; 
	ofile.open(filename); 
	for (int i=0;i<n; i++){
		ofile << fixed <<v(i, lowest) << endl; 
	}
	ofile.close(); 
}
